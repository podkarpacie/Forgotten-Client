# KNOWN ISSUE: startup crash (v7.4.0 build) â€” FOR AI ANALYSIS
...
| Hypothesis | Evidence |
|---|---|
| LuaJIT rolling-2026 incompatibility | Replaced headers + import lib + dll with a fresh **LuaJIT 2.0** build (`v2.0` branch, `msvcbuild.bat dll`) â€” identical crash |
| JIT compiler frames | `luaJIT_setmode(L, 0, LUAJIT_MODE_ENGINE | LUAJIT_MODE_OFF)` at state creation â€” identical crash |
| Link-time code generation miscompile | Disabled WholeProgramOptimization + LTCG â€” identical crash |
...
- A diagnostic in `luaCppFunctionCallback` (`src/framework/luaengine/luainterface.cpp`)
  once logged a **null `funcPtr`** (upvalue userdata NULL) â€” i.e. the C closure was
  invoked **without its upvalue userdata** â€” when called from `g_logger.info`.
- After making the callback null-tolerant (return 0), the crash **moved**: still
  `0xC0000409`, but the null branch is not reached â€” suggesting a second corruption
...
## Key code paths
- `src/framework/luaengine/luainterface.cpp` â€” `pushCppFunction`: placement-`new`
...
- `src/framework/luaengine/luabinder.h` â€” std::function binding templates.
...
1. Does `pushCppFunction`'s stack discipline hold under C++17/v143
   (`newUserdata` â†’ `newTable` â†’ `pushCFunction` â†’ `setField` â†’ `setMetatable` â†’
